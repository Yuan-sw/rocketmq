...
public class RouteInfoManager {
    ...
    /**
     * NameServer与Broker链接空闲时长，默认2分钟，在2分钟内 Nameserver没有收到 Broker的心跳包，则关闭该连接
     */
    ...
    /**
     * topicQueueTable，主题与队列关系，记录一个主题的队列分布在哪些Broker上，
     * 每个Broker上存在该主题的队列个数。QueueData队列描述信息
     */
     ...
     * 根据心跳包的时间得知 broker的状态，该机制也是当一个 Broker 进程假死后，消息生产者无法立即感知，
     * 可能继续向其发送消息，导致失败（非高可用）
     */
     ...
     * @param brokerAddr         broker的IP等地址
     * @param brokerName         broker名称
     * @param brokerId           brokerId
     * @param haServerAddr       HA Broker的地址是Slave从Master拉取数据时链接的地址,由brokerIp2+HA端口构成
     * @param topicConfigWrapper TopicConfigSerializeWrapper比较复杂的数据结构，主要包含了broker上所有的topic信息
...
class BrokerLiveInfo {
    ...
    /**
     * HA Broker的地址是Slave从Master拉取数据时链接的地址,由brokerIp2+HA端口构成
     */
