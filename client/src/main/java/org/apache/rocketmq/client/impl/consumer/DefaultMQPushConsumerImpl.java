...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    public void pullMessage(final PullRequest pullRequest) {
        //首先获取PullRequest的 处理队列ProcessQueue
        ...
        //如果消费者 服务状态不为ServiceState.RUNNING，或当前处于暂停状态，默认延迟3秒再执行(PullMessageService.executePullRequestLater)
        try {
    ...
    /**
     * 看一下延迟执行：其实最终就是将PullRequest,在延迟一定时间后，放入LinkedBlockQueue中，然后继续尝试拉取。
     *
     * @param pullRequest
     * @param timeDelay
     */
    ...
    /**
     * 这个是 pullRequest 立即执行，包含了发送请求结束后重新放入请求队列，以及用rebalance负载形成的请求队列
     *
     * @param pullRequest
     */
    ...
    public void sendMessageBack(MessageExt msg, int delayLevel, final String brokerName)
        ...
        try {
            //根据brokerName获取到broker的路由信息，然后发送至指定的broker上
            ...
            //如果上面发送逻辑失败，则新建一条消息发送给broker，topic是 "%RETRY%" + ConsumeGroupName
    ...
    public synchronized void start() throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                //checkConfig,检查配置信息，主要检查消费者组（consumeGroup）、消息消费方式（messageModel）、
                ...
                //copySubscription 加工订阅信息，将 Map<String /* topic*/, String／＊　ｓｕｂ　ｅｘｔｅｎｄｓ＊／＞转换为Map<String,SubscriptionData>,
                // 同时如果消息消费模式为集群模式，还需要为该消费组对应一个重试主题
                ...
                //创建一个MQClientInstance实例，一个客户端只会持有一个 MQClientInstance 对象，MQClientInstance 无论是消费者还是生产者，都在应用程序这一端
