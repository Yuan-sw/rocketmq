...
public class CommitLog {
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        //获取一个 MappedFile 对象，内存映射的具体实现，获取的是mappedFile集合中的最后一个
        ...
        try {
            ...
            //验证代码上述获取到的 MappedFile 对象，获取一个可用的 MappedFile (如果没有，则创建一个)
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            //如果最终没有获取到mappedFile，则返回异常
            if (null == mappedFile) {
        ...
        //记录此次写入MqppedFile耗时，如若大于500ms，提出警告
     ...
     * 根据配置的策略进行刷盘
     *
     * @param result           MapedFile进行appendMessage的结果
     * @param putMessageResult putMessage的结果
    ...
    public void handleHA(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        //若该Broker为同步双写主用（ SYNC_MASTER），同步双写才需要进行同步复制
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        /**
         * 业务方法，在下面的run方法中循环调用，
         * 每执行完一次 doCommit 等待10毫秒，这也是 waitForRunning 的核心逻辑
         */
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
         ...
         * @param byteBuffer       byteBuffer，NIO 字节容器
         * @param maxBlank         最大可写字节数
         * @param messageExtBatch, backed up by a byte array 消息内部封装实体
         * @return
         */
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
